...
React Native 执行的 React 框架代码，和 React 在 Web 中执行代码是同一份。但是，React Native 渲染的是通用平台视图（宿主视图）而不是 DOM 节点（可以认为 DOM 是 Web 的宿主视图）。 Fabric 渲染器使得渲染宿主视图变得可行。Fabric 让 React 与各个平台直接通信并管理其宿主视图实例。 Fabric 渲染器存在于 JavaScript 中，并且它调用的是由 C++ 代码暴露的接口。在[这篇文章](https://overreacted.io/react-as-a-ui-runtime/#renderers)中有更多关于 React 渲染器的信息。
...
React 组件就是 JavaScript 函数或者类，描述如何创建 React 元素。[这篇博文](https://reactjs.org/blog/2015/12/18/react-components-elements-and-instances.html)中有更多介绍。
